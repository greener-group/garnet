...
def gen_ligand_network(smcs):
    ...
    ligand_network = openfe.ligand_network_planning.generate_lomap_network(
        ligands=smcs, mappers=mapper, scorer=scorer)
...
def get_alchemical_charge_difference(mapping) -> int:
    ...
    This should not depend on whether partial charges have been 
    assigned to the small molecules or not, since only the small
    molecule net charge is used. 
...
def get_settings(ff_path, interpolate_14_exceptions):
    """
    ...
    # Use Garnet FF (also for small molecule) if path given, otherwise use default FF 
    if ff_path is not None:
        settings.forcefield_settings.forcefields = [ff_path]                    
        settings.forcefield_settings.small_molecule_forcefield = ''             
    
    # Interpolate 1-4 exceptions for electrostatic interactions
    settings.alchemical_settings.turn_off_core_unique_exceptions = interpolate_14_exceptions     
    
    return settings


def get_settings_charge_changes(ff_path, interpolate_14_exceptions):
    """
    ...
    # Change settings for run with charge difference                       
    settings.alchemical_settings.explicit_charge_correction = True          # Add alchemical water to handle net charge differences
...
@click.option(
    ...
    type=click.Path(dir_okay=False, file_okay=True, path_type=pathlib.Path),  
    default=None,
    help="Path to XML file created with Garnet (optional)",
)
@click.option(                        
    '--interpolate_14_exceptions',
    is_flag=True,
    help="Specify whether or not to interpolate 1-4 exceptions for unique atoms",
)
def run_inputs(ligands, pdb, cofactors, output, ff_path, interpolate_14_exceptions):
    """
    ...
    ff_path
      A path to an XML file generated with Garnet. If not given,
      simulations will be run with default OpenFE force field settings.
    interpolate_14_exceptions
      Whether or not to interpolate 1-4 exceptions for electrostatic
      interactions between unique atoms and between unique and core atoms.
    """
    # Create the output directory -- default to alchemicalNetwork, fail if it exists
    output.mkdir(exist_ok=False, parents=True)

    # Check setup
    print(f"INFO: Simulations will be run with interpolate_14_exceptions={interpolate_14_exceptions}")
    if str(ff_path) is not None:
        print(f"INFO: Force field defined at {str(ff_path)}")
    ...
    # Deal with partial charges of ligands
    if str(ff_path) is not None:
        # Skip generation of partial charges if using Garnet 
        # (partial charges assigned from input XML file)
        logger.info("Skipping partial charge generation")
    else: 
    ...
    if cofactors is not None:
        errormsg = ('Force field parameters need to be generated for the cofactors.')
        raise ValueError(errormsg)
    ...
    for mapping in ligand_network.edges:
        ...
        if abs(charge_difference) > 1e-3:
            ...
            rfe_settings = get_settings_charge_changes(str(ff_path), interpolate_14_exceptions)
        else:
            rfe_settings = get_settings(str(ff_path), interpolate_14_exceptions)   
    ...
    for transform in alchemical_network.edges:
        transform.to_json(transforms_dir / f"{transform.name}.json")
