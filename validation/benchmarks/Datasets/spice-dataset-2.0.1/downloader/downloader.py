...
def download(config_fp, out_fp):
    with open(config_fp) as input:
        config = yaml.safe_load(input.read())
    max_force = float(config["max_force"]) if "max_force" in config else None
    client = PortalClient("https://ml.qcarchive.molssi.org")

    with h5py.File(out_fp, "w") as outputfile:
        for subset in config["subsets"]:
            ...
            specifications = [
                name for name in dataset.specifications
                if dataset.specifications[name].specification.method == "wb97m-d3bj"
            ]
            while True:
                try:
            ...
            for name in recs_by_name:
                ...
                ds = group.create_dataset(
                    "formation_energy",
                    data=np.array([vars["dft total energy"] - ref_energy for vars in qcvars]),
                    dtype=np.float64,
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out", default="SPICE.hdf5")
    args = parser.parse_args()

    download(args.config, args.out)
