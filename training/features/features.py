# Read molecules and record elements, formal charges, aromatic atoms, ring atoms,
...
# This file is later split into 4 parts to work round GitHub file size limits
out_fp = "features.tsv"
#out_fp = "features_spicetest.tsv"

f = h5py.File("SPICE-2.0.1.hdf5", "r")
#f = h5py.File("SPICE-test.hdf5", "r")

use_graph_shortcut = False
#use_graph_shortcut = True
...
def graph_shortcut(graph, i, j):
    if use_graph_shortcut:
        try:
            spl = nx.shortest_path_length(graph, i, j)
            return spl < 5
    ...
    for i in range(n_atoms):
        ...
        if fci / fci.u != 0:
            ...
            for j in range(n_atoms):
                ...
                if i != j and ei == ej and graph_shortcut(graph, i, j):
                    g2 = deepcopy(graph)
                    g2.remove_node(j)
